...
function format(num, digits) {
  return `${'0'.repeat(digits - String(num).length)}${num}`;
}
...
function timeSpanToString(startDate, endDate) {
  ...
  /* `${format(hours, 2)}:${format(minutes, 2)}:${format(seconds, 2)}.${format(milliseconds, 3)}` */
}
// console.log(
//   timeSpanToString(
...
function angleBetweenClockHands(date) {
  ...
  const handsAngle = 5.45 * hours === minutes ? 0 : 0.5 * (60 * hours - 11 * minutes);
